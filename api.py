...
# ── Mix / pickup: "who's up for games tonight" board ─────────────────────────
# Phase 1 of the 4on4 scheduling engine: players mark what they're up for
# tonight (duel / 2on2 / 4on4) + roughly when; everyone sees who's around and
# how close a game is. Presence/ready-check + in-game notify come in later
# phases. A signup is "active tonight" while its row is active AND was touched in
# the last 18h, so it fades on its own without a cron.
_MIX_GAMES = ("duel", "2on2", "4on4")
_MIX_WINDOW = "18 hours"

def _ensure_mix_schema(cur):
    cur.execute("""CREATE TABLE IF NOT EXISTS mix_signups (
        discord_id   TEXT PRIMARY KEY,
        canonical_id TEXT,
        games        JSONB NOT NULL DEFAULT '[]',   -- subset of duel|2on2|4on4
        from_hour    INT,                            -- earliest local hour 0-23, or NULL = flexible/now
        tz           TEXT,
        note         TEXT,
...
def _mix_rating(cur, canonical_id):
    """Best-known 1on1 rating for a canonical (overall row preferred, else the
    most-played). Prototype uses 1on1 as the balance proxy until a 4on4 mode
    exists. Returns (rating:int|None, matches:int)."""
    if not canonical_id:
        return None, 0
    cur.execute("""SELECT conservative, matches_rated FROM ratings
                   WHERE canonical_id=%s AND mode='1on1'
                   ORDER BY (map='' OR map IS NULL) DESC, matches_rated DESC LIMIT 1""",
                (canonical_id,))
    r = cur.fetchone()
    if not r:
        return None, 0
    return (round(r["conservative"]) if r["conservative"] is not None else None), (r["matches_rated"] or 0)

@app.post("/api/mix/signup")
def mix_signup(authorization: str | None = Header(default=None),
               games: list = Body(..., embed=True),
               from_hour: int | None = Body(default=None, embed=True),
               tz: str | None = Body(default=None, embed=True),
               note: str | None = Body(default=None, embed=True)):
    """Mark myself up for pickup games tonight (Discord login required)."""
    u = _current_user(authorization, required=True)
    picks = [g for g in (games or []) if g in _MIX_GAMES]
    if not picks:
        raise HTTPException(400, "pick at least one of: duel, 2on2, 4on4")
    fh = from_hour if isinstance(from_hour, int) and 0 <= from_hour <= 23 else None
    with pg() as conn:
        cur = conn.cursor()
        _ensure_mix_schema(cur)
        cur.execute("""INSERT INTO mix_signups (discord_id, canonical_id, games, from_hour, tz, note, active, updated_at)
                       VALUES (%s,%s,%s,%s,%s,%s,TRUE, now())
                       ON CONFLICT (discord_id) DO UPDATE SET
                         canonical_id=EXCLUDED.canonical_id, games=EXCLUDED.games,
                         from_hour=EXCLUDED.from_hour, tz=EXCLUDED.tz, note=EXCLUDED.note,
                         active=TRUE, updated_at=now()""",
                    (u["discord_id"], u.get("canonical_id"), json.dumps(picks), fh,
                     tz or u.get("timezone") or (u.get("availability") or {}).get("tz"),
                     (note or "").strip()[:140] or None))
        conn.commit()
    return {"ok": True, "games": picks, "from_hour": fh}

@app.post("/api/mix/leave")
def mix_leave(authorization: str | None = Header(default=None)):
    """Pull myself off tonight's board."""
    u = _current_user(authorization, required=True)
    with pg() as conn:
        cur = conn.cursor()
        _ensure_mix_schema(cur)
        cur.execute("UPDATE mix_signups SET active=FALSE, updated_at=now() WHERE discord_id=%s", (u["discord_id"],))
        conn.commit()
    return {"ok": True}

@app.get("/api/mix/tonight")
def mix_tonight(authorization: str | None = Header(default=None), response: Response = None):
    """Everyone who's up for games tonight: per-game counts + the roster with
    ratings. Optional auth just marks which row is me."""
    me_id = None
    try:
        u = _current_user(authorization, required=False)
        me_id = u.get("discord_id") if u else None
    except Exception:
        pass
    with pg() as conn:
        cur = conn.cursor()
        _ensure_mix_schema(cur)
        cur.execute(f"""SELECT s.discord_id, s.canonical_id, s.games, s.from_hour, s.tz, s.note,
                              s.updated_at, pc.display_name, us.global_name, us.username
                       FROM mix_signups s
                       LEFT JOIN players_canonical pc ON pc.canonical_id = s.canonical_id
                       LEFT JOIN users us ON us.discord_id = s.discord_id
                       WHERE s.active AND s.updated_at > now() - interval '{_MIX_WINDOW}'
                       ORDER BY s.from_hour NULLS FIRST, s.updated_at""")
        rows = cur.fetchall()
        signups, counts = [], {g: 0 for g in _MIX_GAMES}
        for r in rows:
            rating, matches = _mix_rating(cur, r["canonical_id"])
            games = list(r["games"] or [])
            for g in games:
                if g in counts:
                    counts[g] += 1
            signups.append({
                "discord_id": r["discord_id"],
                "name": r["display_name"] or r["global_name"] or r["username"] or "player",
                "canonical_id": r["canonical_id"],
                "rating": rating, "matches": matches,
                "games": games, "from_hour": r["from_hour"], "tz": r["tz"],
                "note": r["note"], "is_me": r["discord_id"] == me_id,
            })
    if response is not None:
        response.headers["Cache-Control"] = "no-store"
    return {"signups": signups, "counts": counts, "total": len(signups)}
...
def _team_summary(team_id):
    ...
    with pg() as conn:
        ...
        cur.execute("""SELECT m.id, m.team_a_id, m.team_b_id, m.score_a, m.score_b, m.winner_id,
                       ...
                       WHERE m.ladder_id=%s AND (m.team_a_id=%s OR m.team_b_id=%s)
        ...
        for r in cur.fetchall():
            ...
            opp_name = r["b_name"] if us_a else r["a_name"]
            opp_id = r["team_b_id"] if us_a else r["team_a_id"]
            opp_logo = r["b_logo"] if us_a else r["a_logo"]
            ...
            for mp in (r["maps"] or []):
                af, bf = mp.get("a_frags"), mp.get("b_frags")
                ours = af if us_a else bf
                theirs = bf if us_a else af
                out_maps.append({"map": mp.get("map"), "our_frags": ours, "their_frags": theirs})
                if ours is not None and theirs is not None:
                    nm = mp.get("map") or "?"
                    if ours > theirs:
                        gw += 1; map_rec[nm]["w"] += 1; margin = ours - theirs
                        if not map_rec[nm]["big_w"] or margin > map_rec[nm]["big_w"]["margin"]:
                            map_rec[nm]["big_w"] = {"score": f"{ours}–{theirs}", "opp": opp_name, "margin": margin}
                    elif theirs > ours:
                        gl += 1; map_rec[nm]["l"] += 1; margin = theirs - ours
                        if not map_rec[nm]["big_l"] or margin > map_rec[nm]["big_l"]["margin"]:
                            map_rec[nm]["big_l"] = {"score": f"{ours}–{theirs}", "opp": opp_name, "margin": margin}
            matches.append({
        ...
        for nm, rec in map_rec.items():
            tot = rec["w"] + rec["l"]
            map_stats.append({"map": nm, "w": rec["w"], "l": rec["l"], "win_rate": _pct(rec["w"], tot),
                              "biggest_w": rec["big_w"], "biggest_l": rec["big_l"]})
        map_stats.sort(key=lambda x: -(x["w"] + x["l"]))

        # ── hub_game_ids for this team's matches → KTX stat join ──
        cur.execute("""SELECT hub_game_ids FROM ladder_matches
                       WHERE ladder_id=%s AND (team_a_id=%s OR team_b_id=%s)""",
                    (ladder_id, team_id, team_id))
        hub_ids = set()
        for r in cur.fetchall():
            hub_ids.update(int(g) for g in (r["hub_game_ids"] or []) if g is not None)
        hub_ids = list(hub_ids)

        team_stats, players = {}, []
        if roster and hub_ids:
            cur.execute("""SELECT mt.hub_game_id AS g, SUM(p.player_frags) AS f, SUM(p.player_deaths) AS d,
                           ...
                           FROM players p JOIN matches mt ON mt.match_id=p.match_id
                           WHERE mt.hub_game_id = ANY(%s) AND p.canonical_id = ANY(%s)
                           GROUP BY mt.hub_game_id""", (hub_ids, roster))
            g = cur.fetchall(); n = len(g)
            if n:
                def avg(k): return round(sum((x[k] or 0) for x in g) / n, 1)
                sf = sum(x["f"] or 0 for x in g); sd = sum(x["d"] or 0 for x in g)
                team_stats = {"maps": n, "eff": _pct(sf, sf + sd), "frags": avg("f"), "deaths": avg("d"),
            ...
            cur.execute("""SELECT p.canonical_id AS cid, COUNT(*) AS maps,
                           ...
                           LEFT JOIN players_canonical pc ON pc.canonical_id=p.canonical_id
                           WHERE mt.hub_game_id = ANY(%s) AND p.canonical_id = ANY(%s)
                           GROUP BY p.canonical_id, display""", (hub_ids, roster))
            for r in cur.fetchall():
                nm = r["maps"] or 0
                def pa(k): return round((r[k] or 0) / nm, 1) if nm else 0
                sf = r["f"] or 0; sd = r["d"] or 0
                players.append({"canonical_id": r["cid"], "name": r["display"], "maps": nm,
...
def debug_movement(names: str = "sane,Blood_Dog", games: int = 18):
    ...
    with pg() as conn:
        ...
        for name in [n.strip() for n in names.split(",") if n.strip()]:
            ...
            # Rank candidates by: exact normalized match first, then by games
            # played (a real established player dwarfs fan/impostor accounts),
            # then shortest name. Length alone wrongly picked "BloodDogFan"
            # (11 chars) over "BLooD_DoG(D_P)" (14) for query "blood_dog".
...
def _metric_card_for_games(C, rows, display, win=13, cid=None, strafe_speed=450, fov=100, mode="1on1"):
    ...
    for gr in rows:
        try:
            ...
            # MAP/ZONE CONTROL — a 4on4 concept (hold space). NOT a duel thing: in
            # 1on1, "control" = ITEM control (RA → +MH → elite cycle RA/YA+MH), which
            # the ECONOMY_STACK dials already capture. So only compute for team modes.
            if mode != "1on1":
        ...
        for name in [n.strip() for n in names.split(",") if n.strip()]:
            ...
            for mp in map_list:
                ...
                per_map_cards[mp] = _metric_card_for_games(C, rows, display, win=win, cid=cid,
                                                           strafe_speed=strafe_speed, fov=fov, mode=mode)
