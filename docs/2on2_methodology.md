...
The temptation is to drop the existing 1on1 TrueSkill pipeline onto 2on2 matches and call it done. TrueSkill literally supports team play out of the box — pass `[player_a, player_b]` vs `[player_c, player_d]` and it'll redistribute the rating delta. We could ship that in an afternoon.

But it would be **wrong**, and silently wrong, in ways that 1on1 isn't. The fundamental difference: in 1on1 the player IS the team — every kill, every armor, every drop is one person's doing. In 2on2 there are two players whose contributions are not separable from the result. A great player carrying a weak partner generates the same scoreboard W as two equal players grinding it out. The TrueSkill update treats both partners identically, which means:

- **Carry players get under-rated.** They win games they "shouldn't" by raw team-skill math, so TrueSkill thinks the team is overperforming and adjusts modestly.
- **Carried players get over-rated.** They ride teammate skill to wins and get equal credit.
- **The "who you played WITH" signal is discarded.** Two wins with a 2300-rated partner shouldn't move you the same as two wins with a 1400 partner. Vanilla TrueSkill ignores this.

2on2 is the *least bad* team mode for naive TrueSkill — only one partner to confound things — but the bias still compounds over hundreds of matches.
...
Run TrueSkill with `[a, b]` vs `[c, d]` as the team composition. Store the per-player μ/σ deltas. This gives us a working leaderboard immediately and is the floor we improve from.

Tune:
- `tau` may need to be higher than 1on1's 5 (~10-15) because 2on2 partner variance adds noise.
- `beta` likely lower (~150-200) since team play is less spiky than 1on1.
- `draw_probability` stays low — 2on2 draws are rare.
...
DDR and frag-differential at the individual level are signals INDEPENDENT of team outcome. A player who racks up a 2.0 DDR in a losing 2on2 effort tells us something TrueSkill doesn't see (the loss says they're worse, the DDR says they were the best player on the server).
...
2. Modify [rate.py](../rate.py) `rate_bucket` to handle team mode: detect `match_mode='2on2'`, group by team_id (or by `team_score_red`/`team_score_blue` columns — confirm schema), call TrueSkill with team lists.
...
- TrueSkill team mode: [Microsoft Research docs](https://www.microsoft.com/en-us/research/project/trueskill-ranking-system/)
