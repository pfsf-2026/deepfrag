...
We use **OpenSkill** (Weng-Lin model via the `openskill.py` package), a Bayesian skill-rating system in the TrueSkill family but with simpler closed-form math and active maintenance. Each player has a Gaussian belief over their true skill — a mean `μ` and standard deviation `σ`. ~~The "conservative" rating shown publicly is `μ − 3σ`~~ **(display convention changed 2026-08-17, decided with Peter):** the public rating is **μ** — the best skill estimate — and rankings sort by it. The smurf/small-sample problem the −3σ floor used to solve is handled by a **provisional gate** instead: a player holds a rank number only when `sigma_effective ≤ 150` AND `unique_opponents ≥ 10`. Everyone else is listed inline, μ visible, rank shown as "—". Why: the floor confused everyone ("why is my rating 230 below my μ?"), punished all 4,600 players to guard against a handful of fresh accounts, and idle-decay silently slid veterans down the board. Under the gate, a simulated fresh 10-0 blowout smurf reaches μ 2402 (would be #2!) but σ 322 → unranked; ghost accounts with stale inflated μ (idle 1-2 years) flip back to provisional as decay re-widens σ past the bar. `conservative` stays in every API payload for tooling. Tier cutoffs (§4) now percentile on **μ over the established population** (`sigma ≤ 150`) so wide-σ ghosts don't drag boundaries.
