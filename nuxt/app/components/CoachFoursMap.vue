<script setup>
// One map's own coach tab (2026-09-21, Peter: "each map has its own tab"). Reads the
// per-map card from the shared 4on4 report: the map's inventory, the player's record
// there, the single "work on this" lever ranked against that map's promotion line,
// the other levers coached at his level on this map, and his last games on it.
    ...
    <template v-else>
      ...
      <template v-else-if="card">
        ...
        <div class="rec" :style="{ '--lc': levelColor }">
          <div class="ri"><span class="n">{{ card.wins }}–{{ card.losses }}</span><span class="l">record · last {{ card.games }} here</span></div>
          <div class="ri"><span class="n" :class="card.above_avg_pg > 0 ? 'pos' : 'neg'">{{ signed(card.above_avg_pg) }}</span><span class="l">above average per game</span></div>
          <div class="ri"><span class="n lv">L{{ card.plays_like || lvl.level }}</span><span class="l">{{ card.plays_like && card.plays_like !== lvl.level ? `plays like L${card.plays_like} here (you are L${lvl.level})` : 'plays like your level' }}</span></div>
          <div class="ri"><span class="n small">{{ card.per_map_baseline ? 'map' : 'pooled' }}</span><span class="l">{{ card.per_map_baseline ? 'baselines from players on this map' : 'not enough players here yet; pooled baselines' }}</span></div>
        </div>
        ...
        <section v-if="card.work_on" class="sec">
          ...
          <div class="card focus">
            <div class="ftitle">{{ card.work_on.label }}<span v-if="card.work_on.is_gate" class="gtag">gate to L{{ nextLevel }}</span></div>
            <div class="cmp">
              <div class="item you"><span class="n">{{ card.work_on.you }}</span>you here</div>
              <div class="item tgt"><span class="n">{{ card.work_on.target }}</span>promotion line</div>
              <div class="item"><span class="n">{{ card.work_on.level_median }}</span>L{{ lvl.level }} median here</div>
              <div v-if="card.work_on.win && card.work_on.loss" class="item"><span class="n">{{ card.work_on.win }} <span class="muted">/</span> {{ card.work_on.loss }}</span>your wins / losses</div>
            </div>
            <p class="why"><b>On this map:</b> {{ card.work_on.map_note || card.work_on.why }}</p>
            <p class="drill"><b>Drill:</b> {{ card.work_on.drill }}</p>
          </div>
        </section>

        <!-- the rest of the syllabus here -->
        <section v-if="rest.length" class="sec">
          <div class="sectitle">📐 Then, on {{ map }}</div>
          <div class="rows">
            <div v-for="l in rest" :key="l.key" class="row">
              <div class="rl"><span class="rlabel">{{ l.label }}</span><span v-if="l.is_gate" class="gtag">gate</span></div>
              <div class="rn mono"><b>{{ l.you }}</b> <span class="arrow">→</span> {{ l.target }} <span class="muted">· L{{ lvl.level }} {{ l.level_median }}</span></div>
              <p v-if="l.map_note" class="rnote">{{ l.map_note }}</p>
            </div>
          </div>
        </section>
        ...
        <section v-if="card.games_cards?.length" class="sec">
          ...
          <div class="gcards">
            <div v-for="g in card.games_cards" :key="g.hub_game_id" class="gcard" :class="{ w: g.win, l: !g.win }">
              <div class="ghead"><span class="gres">{{ g.win ? 'W' : 'L' }}</span><span class="muted small">{{ fmtCoachDate(g.played_at) }}</span></div>
        ...
        <div class="muted small">Targets are {{ map }}'s promotion line when enough players have a baseline here, else the pooled line. Item levers are shares of what the whole game took. <NuxtLink to="/levels">how levels work →</NuxtLink></div>
      </template>
    </template>
  </div>
</template>
...
.ri .l { font-size: 11px; color: var(--fg-3); line-height: 1.3; }
...
.cmp { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 8px; margin: 12px 0; }
...
.item .n { font-size: 20px; font-weight: 800; color: var(--fg); font-variant-numeric: tabular-nums; }
...
.rl { display: flex; gap: 8px; align-items: baseline; }
.rlabel { font-weight: 700; }
.rn { font-size: 13px; font-variant-numeric: tabular-nums; color: var(--fg-2); }
.arrow { color: var(--fg-3); }
.rnote { margin: 0; font-size: 13px; line-height: 1.5; color: var(--fg-2); grid-column: 1 / -1; }
...
@media (min-width: 640px) {
  ...
  .row { grid-template-columns: minmax(0, 1fr) auto; align-items: baseline; }
}
</style>
