...
watch([ready, () => user.value?.canonical_id], ([r, cid]) => {
  if (r && cid) navigateTo(`/coach/player/${encodeURIComponent(cid)}`, { replace: true })
}, { immediate: true })
    ...
    <section class="find">
      ...
      <ul v-if="hits.length" class="hits">
        <li v-for="h in hits" :key="h.canonical_id">
          <NuxtLink :to="`/coach/player/${enc(h.canonical_id)}`" class="hit"><span class="hn">{{ h.display }}</span><span class="hm">{{ h.matches }} games</span></NuxtLink>
        </li>
      </ul>
    </section>
